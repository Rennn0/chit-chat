...
namespace API.Source.Handlers.Authorization
{
    public class TokenHandler : IRequestHandler<AuthRequest, ResponseModelBase<string>>
    {
        ...
        public TokenHandler(UserManager<ApplicationUser> userManager, TokenManager tokenManager, IResend mailer, IMemoryCache cache)
        {
        ...
        public async Task<ResponseModelBase<string>> ExecuteAsync(AuthRequest request)
        {
            ...
            if (user is null || user.Email is null || !await _userManager.CheckPasswordAsync(user, request.Password))
            {
        ...
        {
            if (context.Request.Method != AuthRequest.AuthMethod.Token)
            {
                return;
            }
